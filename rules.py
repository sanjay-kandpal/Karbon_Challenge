...
def total_revenue(data: dict, financial_index):
    ...
    try:
        ...
        revenue = financial.get("pnl", {}).get("lineItems", {}).get("netRevenue")
...
def total_borrowing(data: dict, financial_index):
    ...
    try:
        ...
        total_borrowings = float(long_term_borrowings) + float(short_term_borrowings)
...
def iscr(data: dict, financial_index):
    ...
    try:
        ...
        pbit = financial.get("pnl", {}).get("lineItems", {}).get("profitBeforeInterestAndTax", 0)

        
        depreciation = financial.get("pnl", {}).get("lineItems", {}).get("depreciation", 0)

       
        interest_expenses = financial.get("pnl", {}).get("lineItems", {}).get("interestExpense", 0)

        
        iscr_value = (float(pbit) + float(depreciation) + 1) / (float(interest_expenses) + 1)
...
def borrowing_to_revenue_flag(data: dict, financial_index):
    ...
    if borrowing_ratio is not None and borrowing_ratio <= 0.25:
        return FLAGS.GREEN  
    else:
        return FLAGS.AMBER
